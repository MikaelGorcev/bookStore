...
import {TypeBook, TypeStateForBook} from "../../reducers/reducer";
...
const mapStateToProps=( {stateForBook:{books,loading,error} }:{stateForBook:TypeStateForBook} ):TypeStateForBook=>{ 
...
const mapDispatchToProps=(dispatch:Tdispatch,{bookStService}:{bookStService:IBookStoreService})=>{
  ...
  return {fetchBooks:fetchBooks(dispatch,bookStService),
...
export default withBookService<IBookStoreServiceConnect>(
    connect<TypeStateForBook,IDispatchProps,IBookStoreServiceConnect>(mapStateToProps,mapDispatchToProps)(BooklistContainer)
    )